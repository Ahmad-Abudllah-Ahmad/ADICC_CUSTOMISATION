...
test("default mode → product brand, no company, no credit, prefixed cover title", () => {
  ...
  assert.equal(b.coverTitle, `${OT_NAME} · Marked Set`);
});
...
test("clear-label profile without a name → brandName degrades to product brand, company kept", () => {
...
// resolver hands brandName to each CSV, defaulting to the product brand when unbranded
test("CSV export titles carry the brand name (default product brand)", () => {
  assert.match(totalsToCsv([], "Proj"), new RegExp(`^# Proj — ${OT_NAME} report`));
  assert.match(totalsToCsv([], "Proj", null, null, null, null, null, "345 Flooring"), /^# Proj — 345 Flooring report/);
  assert.match(shapesToCsv([], "Proj"), new RegExp(`^# Proj — ${OT_NAME} shapes`));
  assert.match(shapesToCsv([], "Proj", "345 Flooring"), /^# Proj — 345 Flooring shapes/);
  assert.match(rfisToCsv([], [], "Proj"), new RegExp(`^# Proj — ${OT_NAME} RFI log`));
