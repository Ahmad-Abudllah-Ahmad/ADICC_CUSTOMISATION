...
1. **Load the sample.** On the opening screen, click **Load sample plan** — a real medical-center floor finish plan. (Your own plans: drag a PDF anywhere onto the page.) Before a sheet is up, and while it is reading, the canvas shows the ADICC loading wordmark — the Workspace Bar, Measure Rail, Takeoffs Drawer, and takeoff value wait until the sheet is ready. Once the sheet is up, the takeoff value sits above the minimap on the bottom-right view dock, next to a session trend line — hover a point for the value and delta.
...
3. **Pick a condition.** A fresh workspace ships with a starter set of flooring conditions — CPT-1, LVT-1, CT-1, and friends. Press `1` to arm the first (the number keys answer in list order until you pin your own palette), or reveal the **Takeoffs Drawer** from the right edge and click one.
...
- **Minimap** (on by default, map icon in the zoom cluster): fixed-height overview — drag to pan, scroll or pinch on it to zoom the sheet toward that point. The cobalt box is the current view. Toggle persists per browser.
...
The **contrast** button on the left rail inverts the sheet pixels themselves — a true negative print, white linework on black, not a CSS filter — with hatches retuned to stay legible. Chrome light/dark is the sun/moon control next to **Pan** in the center toolbar; it does not invert the sheet. Hover any canvas chrome icon for its name and shortcut — the label appears immediately (under the toolbar, to the right of the left rail, above zoom/undo, or beside the right drawer). The invert setting persists per browser, and exports follow it: a dark canvas produces a dark Marked Set PDF.

The three main chrome surfaces are the **Measure Rail** (left), **Workspace Bar** (top), and **Takeoffs Drawer** (right). In the ADICC host, the secondary bar owns the project desk: **Files, Summary, Sheets, Markups, Stamps, and RFIs** sit to the left of **Tools** and **View**. The whole secondary bar waits for the sheet data to finish loading, then enters together from above, so its controls cannot be used against an unready canvas. Each label opens the original live content in a bounded menu shell; the retired Summary and Files rail triggers are replaced by a symbol-only ADICC mark. **Summary** is the Takeoff Register. Its overview shows live Floor / Wall / Linear / Count totals (zero totals are omitted). Each finish is a color-owned material strip with a quantity and an explicit **Review rooms** control. Expanded locations are numbered and always show two actions: the target locates the measurement on the plan, and the eye shows or hides it. Finish and floor eyes are also permanent—visibility never depends on hover. The floor-header eye hides that floor’s list and stays on the header as a filled red control so it can be turned back on. Click a swatch to recolor; double-click a finish code to rename it. Same finish tags on a floor combine. **Expand / Collapse** opens or closes all room details. That mark is also the rail’s drag handle, with Reset directly underneath and a divider before the measurement tools. A plain press/release is a no-op; drag arms only after 5px of movement so a click does not jump the rail. Once armed, the rail follows the pointer with compositor transforms and stays clamped inside the canvas viewport — it does not cross into the host navbar. Reset returns it smoothly to the centered default. The dropdown has its own title and close control and intentionally omits the dock’s mixed Files / Summary / Sheets switcher — the secondary navbar itself performs that switching. On the Measure Rail dock, that switcher is a titleblock: the open section is the title (with its count), the other sections sit as a quiet index, and Close is a word. Clicking the active label again closes its menu, and the active underline follows panel changes made inside the canvas. **Tools** is a compact checklist: a tick means visible, and clicking the whole row manually shows or hides that surface with directional motion. Under **Measure Rail**, **Small / Medium / Large** changes only the left-rail icon size (Small is 5% tighter than the original rail; Medium and Large are short steps up); hide/show and the `Alt+Shift+1` shortcut stay the same. The choice persists per browser. The Measure Rail, Sheets, and Chat controls float as one adaptive stack, centered between the secondary navigation and the screen bottom. There is no hover-based reveal, idle hide, or lock control.

The adjacent host **View** checklist controls presentation without changing takeoff data: **Takeoff Value** and **Live Readout** (stacked above the minimap on the bottom-right view dock), **Minimap**, **Rulers**, **Drafting Grid**, and the bottom-center **Scale Bar**. Minimap and Scale Bar start on; Takeoff Value, Live Readout, Rulers, and Grid start off. Hidden live overlays remain mounted and keep receiving current quantities. Rulers run along the left and bottom edges, use calibrated feet/meters when a scale exists, and fall back to screen-pixel divisions before calibration. The grid is a low-intensity cobalt drafting grid that follows the sheet and changes to a pale screen blend on an inverted sheet. These choices persist per browser. The menu prints the working shortcuts on the right: `Alt+Shift+1…3` toggles Measure Rail / Workspace Bar / Takeoffs Drawer; `Alt+Shift+4…9` toggles Takeoff Value / Live Readout / Minimap / Rulers / Drafting Grid / Scale Bar.

The **Fullscreen** control at the end of the bottom-right view controls expands the embedded canvas across the ADICC host window. The primary ADICC header hides, while the secondary **Files / Summary / Sheets / Markups / Stamps / RFIs / Tools / View** bar stays available at the top. The button changes to exit, and `Esc` restores the complete workspace. When the canvas runs standalone, the same control uses browser fullscreen. Clicking outside **Tools** or **View** closes the open menu. Opening the Takeoffs Drawer slides it over the minimap and view controls with no scrim; those stay in the bottom-right corner, and the sheet stays clickable underneath. Close the drawer with its **×** or **Tools → Takeoffs Drawer** again.
...
The **Takeoffs Drawer** slides in from the right edge (it starts collapsed; the palette band is the primary surface). In the ADICC host, **Tools → Takeoffs Drawer** opens and closes it — there is no dimmed backdrop, and clicking the sheet does not dismiss it. Close with the panel **×** or by toggling that Tools row again. Four tabs:
...
The live readout sits above the minimap, stacked under the takeoff-value figure, as text inside thin crop-marks — no plate, no glow. It stays visible while idle (sheet floor, or an em dash before scale) and switches to the live measure as soon as you trace. A paper halo and high-contrast corner marks keep both figures off the sheet in UI dark and invert. It tracks the armed tool: totals for the tracing tools, `W × H · SF · SY` for Rectangle, wall SF at the condition height for Surface Area, and running One-Click selection totals. Wall-height and door-opening editors stay clickable when a wall is selected. Any run or side that reaches **12′ turns the chip amber** — the roll-width warning rides every tool that measures a length.
