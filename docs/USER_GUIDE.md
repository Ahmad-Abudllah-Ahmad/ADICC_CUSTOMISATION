...
The three main chrome surfaces are the **Measure Rail** (left), **Workspace Bar** (top), and **Takeoffs Drawer** (right). In the ADICC host, the secondary bar owns the project desk: **Files, Summary, Sheets, Markups, Stamps, and RFIs** sit to the left of **Tools** and **View**. The whole secondary bar waits for the sheet data to finish loading, then enters together from above, so its controls cannot be used against an unready canvas. Each label opens the original live content in a bounded menu shell; the retired Summary and Files rail triggers are replaced by a symbol-only ADICC mark. That mark is also the rail’s drag handle, with Reset directly underneath and a divider before the measurement tools. A plain press/release is a no-op; drag arms only after 5px of movement so a click does not jump the rail. Once armed, the rail follows the pointer with compositor transforms and stays clamped inside the canvas viewport — it does not cross into the host navbar. Reset returns it smoothly to the centered default. The dropdown has its own title and close control and intentionally omits the dock’s mixed Files / Summary / Sheets tab strip — the secondary navbar itself performs that switching. Clicking the active label again closes its menu, and the active underline follows panel changes made inside the canvas. **Tools** is a compact checklist: a tick means visible, and clicking the whole row manually shows or hides that surface with directional motion. The Measure Rail, Sheets, and Chat controls float as one adaptive stack, centered between the secondary navigation and the screen bottom. There is no hover-based reveal, idle hide, or lock control.
