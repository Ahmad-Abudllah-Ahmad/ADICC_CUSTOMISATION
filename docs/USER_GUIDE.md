...
The three main chrome surfaces are the **Measure Rail** (left), **Workspace Bar** (top), and **Takeoffs Drawer** (right). In the ADICC host, **Tools** is a compact checklist: a tick means visible, and clicking the whole row manually shows or hides that surface with directional motion. The Measure Rail, Sheets, and Chat controls float as one adaptive stack, centered between the secondary navigation and the screen bottom. There is no hover-based reveal, idle hide, or lock control.
...
The **Fullscreen** control at the end of the bottom-right view controls fills the window with the canvas: the whole ADICC chrome (primary header and Tools / View) hides. A custom top-center toast — **ADICC — To restore, press Esc** — appears, then fades. The button changes to exit, and `Esc` restores the chrome. Clicking outside **Tools** or **View** closes the open menu when those bars are visible. Opening the Takeoffs Drawer slides it over the minimap and view controls with no scrim; those stay in the bottom-right corner, and the sheet stays clickable underneath. Close the drawer with its **×** or **Tools → Takeoffs Drawer** again.
