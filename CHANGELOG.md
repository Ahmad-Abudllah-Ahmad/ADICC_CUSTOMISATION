...
### Added
- **Canvas tool catalog tests** pin every measure / cut / markup / mode tool’s select-vs-commit contract and which Layers kind (if any) a commit creates (`web/test/canvasTools.test.ts`). Live `commitPoly` / `commitLinear` / `commitSurface` / `commitCount` / `commitWallTrace` roles and scale/height gates, `performClick` routing, gallery `G`, and USER_GUIDE §15 shortcuts are source-pinned against TakeoffCanvas so a role drift fails the suite.

### Changed
- **Sheets tab tools no longer blank the canvas.** Split / Eye / Close / focusing a sheet already in the pair keep painted pages on screen and only raster a page that is not cached. Row click in a pair focuses that panel; Eye on the only visible sheet is disabled. The selected row uses a cobalt bar instead of a Selected pill.
- **Open-sheets list is a simple row list.** Idle rows stay quiet; the current sheet is the cobalt row with a **Now** mark. Eye / split / close sit in a fixed rail; hover labels open to the left so they don't clip or spawn a horizontal scrollbar. Search is an underline field, not the Files pill.
- **Minimap is on by default** (map icon in the zoom cluster). Drag the overview to pan — the viewport box tracks via the same CSS transform as the sheet, so it doesn’t redraw the thumbnail on every move. Off/on persists per browser.
- **Illustrator-style Layers panel** on the left rail (2nd from top) lists every takeoff on the open sheets. Eye hides a shape, a group, or a whole sheet folder; lock blocks move / vertex / flip / copy / duplicate / reassign / delete the same way; Ctrl/Shift-click multi-selects and draws selection chrome on every picked shape; **Group** / **Ungroup** (`Ctrl+Shift+G` / `Ctrl+Shift+U`) nest groups (parent select moves all descendants). Drag a group into a group keeps it nested. The row strip and selection square follow **live paint** (custom appearance → condition colour → kind fallback). The tree, hide map, and lock map persist with the takeoff (`layer_tree` / `layer_hidden` / `layer_locked`) — including tree-only edits (group/lock/hide) and empty-sheet hide/lock (`sheet::<key>`). Esc and an empty-sheet click clear the layer pick; fly-to / commit retargets the pick unless the shape was already in a multi-pick. Group stays on one sheet (Select all is the focused sheet). Deleting a grouped shape and `⌘Z` puts it back in the group.
...
- **Left-panel tabs scroll sideways** when Files / Sheets / Markups / Stamps / RFIs don't fit — drag or wheel. Close is a pinned end-cap; ‹ › sit inside the tab track so they never stack on the last label.
- **Layers lives only on its rail icon.** The Files desk tab strip is Files / Sheets / Markups / Stamps / RFIs — the duplicate Layers tab (and its double-click floating window) is gone.
...
- **Toolbar is tools only.** Auto-Takeoff and Takeoff Tool Palette sit in the center tools pill. Takeoff value (with crop-mark focus) appears top-left only once the total is above zero; the live readout appears top-right only while a measure is live. Toolbar icons share one 30px row. Canvas chrome uses instant hover labels (no delayed browser tooltip): below the toolbar, right of the left rail / Sheets / Chat, above zoom/undo, left of the Takeoffs hamburger. Auto-Takeoff stays a readable green ghost in both themes.
- **Takeoff value and live readout are text plus thin crop-marks.** No plate, no glow. Corners are black in light chrome and white in dark chrome (and on an inverted sheet). The bottom scale bar still follows the sheet paper.
...
### Fixed
- **Layers Ctrl/Shift/Alt-click match Illustrator.** The row, the name, and the target circle all go through the same pick: Ctrl/Cmd toggles, Shift ranges, Alt picks that row. Alt-click eye solos (again shows all); Alt-click lock isolates; Alt-click the triangle expands the nested group. Ctrl/Cmd+A in the panel picks the focused sheet. On the canvas with Select, Shift adds and Ctrl/Cmd toggles — wall-area join no longer steals those clicks unless you are actually joining endpoints.
- **Layers eye, lock, and + work on sheet folders.** Paired sheets with no takeoff still listed two folders while Hide returned without doing anything and lock was a dead spacer; the row drag handle also swallowed clicks. Eye / lock now toggle the whole sheet (and stick for new takeoff on that sheet); New Group lands on the folder you clicked. The footer counts sheets/groups separately from layers.
- **Side-by-side can't make a one-sheet "pair" anymore.** Splitting the only/active sheet with itself (from the Sheets list or the Files side-by-side button) used to leave an invalid single-sheet group and a confusing pair UI. The split button is now disabled with a reason when it would pair a sheet with itself or exceed the 4-sheet cap, and `toggleInGroup` refuses both cases with a one-line hint.
