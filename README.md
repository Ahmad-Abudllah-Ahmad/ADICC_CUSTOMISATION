...
Zoom in and the linework stays **razor-sharp**: past ~1.15× zoom **times your display's pixel ratio** (so it kicks in sooner on a 2×/Retina screen than a 1× monitor) the visible region re-renders straight from the PDF vectors at your current zoom — Bluebeam/AutoCAD-style — instead of magnifying a fixed bitmap, so fine callouts and hatching never blur. It engages after a brief pause in scrolling/pinching rather than on every frame, so a continuous zoom stays on the fast base layer until you stop moving. It overlays just what's on screen, so there's no giant full-sheet bitmap to hold. A taller, clean **minimap** (on by default, map icon in the zoom cluster) lets you drag the overview to pan. The ADICC mark at the top of the Measure Rail is its drag handle; a plain click is a no-op, actual movement starts the compositor-only drag, the canvas and minimap stay frozen in place, and an out-of-canvas drop eases back to the nearest valid edge. Chrome light/dark is the sun/moon next to **Pan**. In an ADICC host, **Tools** and **View** use compact IDE-style checklist rows: checked means visible, clicking the row toggles it, and `Alt+Shift+1…9` provides matching shortcuts for the three chrome surfaces and six view overlays. **Tools → Measure Rail** also has Small / Medium / Large for the left-rail icon size (Small is slightly tighter than the original rail). The secondary bar waits for the sheet to be ready before entering, so these controls cannot outrun project loading. Hidden live overlays keep calculating. The expand control beneath the minimap gives the drawing the full host window, hides the primary ADICC header, and keeps the secondary project / Tools / View bar available at the top; `Esc` restores the complete workspace. Standalone canvas mode still uses browser fullscreen. A separate **dark view** (contrast, left rail) inverts the sheet itself — a true negative print, white linework on black, not a CSS filter — with hatches retuned so takeoffs read as well at night as they do at noon. And a visual **gallery** (`G`) to pick and open sheets, **Regroup** to restore a side-by-side composition in one click, per-sheet **Hi-Res** base rendering, **Snap** (beta) to plan lines and corners, and a separate **markup layer** (revision clouds, callouts, text notes) that's never counted in the totals.
...
| **Layers** | One row per takeoff on the left rail: hide, lock, nested groups, Illustrator-style Ctrl/Shift/Alt-click |
| **Project desk** | Files, Summary, Sheets, Markups, Stamps, and RFIs open from the ADICC host secondary bar using the same live canvas state; the retired Summary/Files rail triggers are replaced by the symbol-only ADICC mark |
| **View** | Host controls for takeoff value, live readout, minimap, left/bottom rulers, drafting grid, and scale bar; chrome light/dark remains separate from sheet **invert (negative print)** |
